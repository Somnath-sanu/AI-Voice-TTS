...
R2_BUCKET_NAME = "ai-voice-clones"
R2_ACCOUNT_ID = "TIGRIS"
R2_MOUNT_PATH = "/r2"
r2_bucket = modal.CloudBucketMount(
    R2_BUCKET_NAME,
    bucket_endpoint_url=f"https://t3.storage.dev",
    secret=modal.Secret.from_name("tigris"),
    read_only=True,
)
...
@app.cls(
    ...
    secrets=[
        modal.Secret.from_name("hf-token"), # Why we need hugging face token?
        modal.Secret.from_name("chatterbox-api-key"), # Any random api key, its just used for verfication. (chatterbox-client.ts)
        modal.Secret.from_name("tigris"),
    ],
